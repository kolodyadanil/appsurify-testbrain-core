...
class EmailAddress(models.Model):
    ...
    def send_confirmation(self, request=None, signup=False, organization=None):
        if app_settings.EMAIL_CONFIRMATION_HMAC:
            confirmation = EmailConfirmationHMAC(self)
        else:
            confirmation = EmailConfirmation.create(self)
        confirmation.send(request, signup=signup, organization=organization)
        return confirmation
...
class EmailConfirmation(models.Model):
    ...
    def send(self, request=None, organization=None, signup=False):
        get_adapter(request).send_confirmation_mail(request, self, signup, organization=organization)
        self.sent = timezone.now()
        self.save()
        signals.email_confirmation_sent.send(sender=self.__class__, request=request, confirmation=self, signup=signup)


class EmailConfirmationHMAC(object):
    ...
    def confirm(self, request, organization=None):
        if not self.email_address.verified:
            email_address = self.email_address
            get_adapter(request).confirm_email(request, email_address, organization=organization)
            signals.email_confirmed.send(sender=self.__class__, request=request, email_address=email_address)
            return email_address

    def send(self, request=None, signup=False, organization=None):
        get_adapter(request).send_confirmation_mail(request, self, signup, organization=organization)
        signals.email_confirmation_sent.send(sender=self.__class__, request=request, confirmation=self, signup=signup)
