...
from ..compat import authenticate, is_authenticated, reverse, validate_password, has_usable_password
from ..utils import (
    build_absolute_uri,
...
class DefaultAccountAdapter(object):
    ...
    def format_email_subject(self, subject, organization=None):
    ...
    def render_mail(self, template_prefix, email, context, organization=None):
        """
        ...
        subject = self.format_email_subject(subject, organization=organization)
    ...
    def send_mail(self, template_prefix, email, context, organization=None):
        msg = self.render_mail(template_prefix, email, context, organization=organization)
        try:
            msg.send()
        except Exception as exc:
            pass

    def get_email_confirmation_url(self, request, emailconfirmation, organization=None):
        ...
        ret = build_absolute_uri(request, url, organization=organization)
        return ret

    def send_confirmation_mail(self, request, emailconfirmation, signup, organization=None):
        ...
        #
        # if organization:
        #     current_site = organization.site
        ...
        activate_url = self.get_email_confirmation_url(request, emailconfirmation, organization=organization)
        ...
        if signup and not organization:
            email_template = 'account/email/email_confirmation_invite'
        else:
            email_template = 'account/email/email_confirmation'

        self.send_mail(email_template, emailconfirmation.email_address.email, ctx, organization=organization)

    ########################################################################

    def get_login_redirect_url(self, request, organization=None):
        """
        ...
        if not has_usable_password(request.user):
            redirect_url = app_settings.PASSWORD_CHANGE_REDIRECT_URL
        else:
            redirect_url = resolve_url(settings.LOGIN_REDIRECT_URL)

        if organization:
            redirect_url = build_absolute_uri(request, location=redirect_url, protocol=None, organization=organization)
        return redirect_url
    ...
    def populate_username(self, request, user):
        ...
        if app_settings.USER_MODEL_USERNAME_FIELD:
            user_username(
                user,
                username or self.generate_unique_username([first_name, last_name, email, username, 'user'])
            )
    ...
    def clean_password(self, password, user=None):
        ...
        if min_length and len(password) < min_length:
            raise ValidationError(_("Password must be a minimum of {0} characters.").format(min_length))
        validate_password(password, user)
        return password

    def validate_unique_email(self, email):
        if email_address_exists(email):
            raise ValidationError(self.error_messages['email_taken'])
        return email
    ...
    def confirm_email(self, request, email_address, organization=None):
        """
    ...
    def is_safe_url(self, url):
        ...
        return is_safe_url(url, settings.ALLOWED_HOSTS)

    def respond_user_inactive(self, request, user):
        # return HttpResponseRedirect(reverse('account_inactive'))
        return Response(data={'detail': 'Current user is inactive. Please contact with administrator or support.'},
                        status=status.HTTP_201_CREATED)

    def respond_email_verification_sent(self, request, user, organization=None):
        return Response(data={'detail': 'We have sent an e-mail to you for verification. '
