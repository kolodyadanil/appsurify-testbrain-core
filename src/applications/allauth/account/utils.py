...
from applications.allauth.compat import base36_to_int, int_to_base36
...
def create_token(token_model, user, serializer=None):
...
def get_login_redirect_url(request, url=None, redirect_field_name="next", organization=None):
    ...
    redirect_url = (
        ...
        get_adapter(request).get_login_redirect_url(request, organization=organization))
    return redirect_url
...
def perform_login(request, user, email_verification, redirect_url=None, signal_kwargs=None, signup=False, organization=None):
    """
    Keyword arguments:
    ...
    if email_verification == EmailVerificationMethod.NONE:
        ...
        if not has_verified_email and signup:
            send_email_confirmation(request, user, signup=signup, organization=organization)
    elif email_verification == EmailVerificationMethod.MANDATORY:
        if not has_verified_email:
            send_email_confirmation(request, user, signup=signup, organization=organization)
            return adapter.respond_email_verification_sent(request, user, organization=organization)
        if has_verified_email and signup:
            send_email_confirmation(request, user, signup=signup, organization=organization)
            return adapter.respond_email_verification_sent(request, user, organization=organization)

    try:
        if signup and not app_settings.LOGIN_ON_SIGNUP:
            pass

        else:
            adapter.login(request, user)

        response = HttpResponseRedirect(get_login_redirect_url(request, redirect_url, organization=organization))

        if signal_kwargs is None:
            signal_kwargs = {}

        signals.user_logged_in.send(sender=user.__class__, request=request, user=user, **signal_kwargs)
        adapter.add_message(request, messages.SUCCESS, 'Successfully signed in as {{name}}', {'user': user})
...
def complete_signup(request, user, email_verification, success_url, signal_kwargs=None, organization=None):
    if signal_kwargs is None:
        signal_kwargs = {}
    signals.user_signed_up.send(sender=user.__class__, request=request, user=user, **signal_kwargs)
    return perform_login(request, user, email_verification=email_verification, signup=True, redirect_url=success_url, signal_kwargs=signal_kwargs, organization=organization)
...
def send_email_confirmation(request, user, signup=False, organization=None):
    """
    ...
    if email:
        try:
            ...
            if not email_address.verified or signup:
                ...
                if send_email:
                    email_address.send_confirmation(request, signup=signup, organization=organization)
        ...
        if send_email:
            get_adapter(request).add_message(request, messages.INFO, 'Confirmation e-mail sent to {{email}}.', {'email': email})

    if signup:
        get_adapter(request).stash_user(request, user_pk_to_url_str(user))


def sync_user_email_addresses(user, force=False):
    """
    ...
    if email and not EmailAddress.objects.filter(user=user, email__iexact=email).exists():
        ...
        EmailAddress.objects.create(user=user, email=email, primary=force, verified=force)
