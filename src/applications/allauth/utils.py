...
def generate_unique_username(txts, regex=None):
    from .account.app_settings import USER_MODEL_USERNAME_FIELD
    from .account.adapter import get_adapter
    from applications.allauth.account.utils import filter_users_by_username
            ...
            try:
                ...
                if isinstance(f, DateTimeField):
                    ...
                    try:
                        ...
                        raise ImproperlyConfigured("Unable to auto serialize field '{}', custom serialization override required".format(k))
...
def build_absolute_uri(request, location, protocol=None, organization=None):
...
def send_registration_email(organization, user, **kwargs):
    from .account import app_settings as account_settings

    proto = kwargs.get('proto', 'http')
    if account_settings.DEFAULT_HTTP_PROTOCOL == 'https':
        proto = account_settings.DEFAULT_HTTP_PROTOCOL

    organization_url = '{proto}://{company_domain}'.format(proto=proto, company_domain=organization.site.domain)
    ...
    subject = render_to_string('account/email/email_organization_signup_subject.txt', context).strip()

    bodies = {}
    for ext in ['html', 'txt']:
        try:
            template_name = 'account/email/email_organization_signup_message.{}'.format(ext)
    ...
    if 'txt' in bodies:
        msg = EmailMultiAlternatives(subject, bodies['txt'], from_email, [user.email])
        if 'html' in bodies:
            msg.attach_alternative(bodies['html'], 'text/html')
    else:
        msg = EmailMessage(subject, bodies['html'], from_email, [user.email])
