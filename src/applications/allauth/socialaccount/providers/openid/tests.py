...
from applications.allauth.tests import Mock, TestCase, patch
...
class OpenIDTests(TestCase):
    ...
    def test_login(self):
        ...
        with patch('applications.allauth.socialaccount.providers'
    ...
    def test_login_with_extra_attributes(self):
        with patch('applications.allauth.socialaccount.providers.openid.views.QUERY_EMAIL',
                   True):
            resp = self.client.post(reverse(views.login),
                                    dict(openid='http://me.yahoo.com'))
        assert 'login.yahooapis' in resp['location']
