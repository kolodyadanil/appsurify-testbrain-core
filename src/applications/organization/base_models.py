...
from .managers import ActiveOrgManager
...
class OrgMeta(ModelBase):
    ...
    This is particularly useful for custom organization that can avoid
    ...
    def update_org(cls, module):
        ...
        try:
            ...
            cls.module_registry[module]['OrgModel'].add_to_class('users', models.ManyToManyField(USER_MODEL, through=cls.module_registry[module]['OrgUserModel'].__name__, related_name='%(app_label)s_%(class)s'))
...
class AbstractBaseOrganization(UnicodeMixin, models.Model):
    ...
    name = models.CharField(max_length=255, help_text=_('The name of the organization'))
...
class AbstractBaseOrganizationUser(UnicodeMixin, models.Model):
    ...
    It is possible for a User to be a member of multiple organization, so this
    ...
    def __unicode__(self):
        return '{0} ({1})'.format(self.user.get_full_name() if self.user.is_active else self.user.email, self.organization.name)
