...
env = dict(
    PGHOST='testbrain-prod.postgres.database.azure.com',
    PGPORT='5432',
    PGDATABASE='testbrain',
    PGUSER='testbrain',
    PGPASSWORD='Pxc7x3asJyHeSfn23',
)
...
for test_suite_id, project_id, org_name in test_suites:
    ...
    directory = os.path.join(os.getcwd(), str(org_name), str(project_id))
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = os.path.join(directory, "{}.csv".format(test_suite_id))

    try:
        cmd = ['/usr/local/Cellar/libpq/14.1/bin/psql', '-c',
               "\copy ({sql}) To '{filename}' With CSV DELIMITER ',' HEADER".format(sql=sql, filename=filename)]
