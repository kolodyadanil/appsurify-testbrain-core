...
LOGGING_CONFIG = {
    ...
    'handlers': {
        ...
        'file': {
            ...
            'formatter': 'default',
            'filename': './testbrain-datasets.log',
...
log.debug("initialize database server connection")
conn = psycopg2.connect(dbname="testbrain", user="testbrain",
                        password="Pxc7x3asJyHeSfn23",
                        host="testbrain-prod.postgres.database.azure.com")

log.debug("connect to database success")

sql_test_suites = """
SELECT "testing_testsuite"."id", "testing_testsuite"."project_id", "organization_organization"."name" 
FROM "testing_testsuite" 
INNER JOIN "project_project" ON ("testing_testsuite"."project_id" = "project_project"."id") 
INNER JOIN "organization_organization" ON ("project_project"."organization_id" = "organization_organization"."id") 
WHERE "organization_organization"."name" IN ('vonage', 'Juriba', 'forbes');
"""
cursor = conn.cursor()
cursor.execute(sql_test_suites)
test_suites = cursor.fetchall()
cursor.close()

log.info("start storing datasets for test_suites. Total: {}".format(len(test_suites)))
for test_suite_id, project_id, org_name in test_suites:
    log.info("TEST_SUITE({}) start processing".format(test_suite_id))
    start_time = time.time()
    log.debug("prepare SQL query for TEST_SUITE id = {}".format(test_suite_id))
    sql = sql_template.format(test_suite_id=test_suite_id)
    cursor = conn.cursor("ml-datasets")
    cursor.itersize = 5000
    log.debug("execute SQL query for TEST_SUITE id = {}".format(test_suite_id))

    directory = os.path.join(str(org_name), str(project_id))
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = os.path.join(directory, "{}.csv".format(test_suite_id))

    try:
        cursor.execute(sql)
        with open(filename, "wb") as csvfile:
            columns = ['test_changed', 'test_names', 'test_classes_names', 'test_areas', 'test_associated_areas',
                       'test_associated_files',
                       'test_dependent_areas', 'test_similarnamed', 'test_area_similarnamed', 'commit_rework',
                       'commit_riskiness',
                       'commit_areas', 'commit_files', 'defect_closed_by_caused_by_commits_files',
                       'defect_closed_by_caused_by_commits_areas', 'defect_closed_by_caused_by_commits_dependent_areas',
                       'defect_closed_by_caused_by_intersection_areas', 'defect_closed_by_caused_by_intersection_files']
    ...
    end_time = time.time()
    log.info("TEST_SUITE({}) finish processing. (Time: {})".format(test_suite_id, int(end_time - start_time)))
