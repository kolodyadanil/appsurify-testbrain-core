...
class TestUtils(TestBase):
    ...
    def test_basics(self):
        ...
        assert to_hex_sha(to_bin_sha(NULL_HEX_SHA)) == NULL_HEX_SHA.encode("utf-8")

    def _cmp_contents(self, file_path, data):
        # raise if data from file at file_path
        # does not match data string
        with open(file_path, "rb") as fp:
            assert fp.read() == data.encode("utf-8")
    ...
    def test_lockedfd(self):
        ...
        with open(my_file, "wb") as my_file_fp:
            my_file_fp.write(orig_data.encode("utf-8"))
        ...
        try:
            ...
            # write data and fail
            os.write(wfd, new_data.encode("utf-8"))
            lfd.rollback()
            ...
            assert os.read(rfd, len(orig_data)) == orig_data.encode("utf-8")
            ...
            wfdstream.write(new_data.encode("utf-8"))
            lfd.commit()
            assert not os.path.isfile(lockfilepath)
