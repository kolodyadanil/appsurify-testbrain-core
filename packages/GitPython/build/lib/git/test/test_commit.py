...
def assert_commit_serialization(rwrepo, commit_id, print_performance_info=False):
    ...
    for cm in rwrepo.commit(commit_id).traverse():
        ...
        assert_equal(istream.hexsha, cm.hexsha.encode('utf-8'))
...
class TestCommit(TestBase):
    ...
    def test_rev_list_bisect_all(self):
        ...
        commits = Commit._iter_from_process_or_stream(self.rorepo, StringProcessAdapter(revs.encode('utf-8')))
        expected_ids = (
...
-----END PGP SIGNATURE-----"""
        ...
        cstream = BytesIO()
        cmt._serialize(cstream)
        assert re.search(r"^gpgsig <test\n dummy\n sig>$", cstream.getvalue().decode('utf-8'), re.MULTILINE)
        ...
        cstream = BytesIO()
        cmt._serialize(cstream)
        assert not re.search(r"^gpgsig ", cstream.getvalue().decode('utf-8'), re.MULTILINE)
