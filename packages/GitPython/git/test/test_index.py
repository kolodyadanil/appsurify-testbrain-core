...
class TestIndex(TestBase):
    ...
    def test_index_mutation(self, rw_repo):
        ...
        if not is_win:
            for target in ('/etc/nonexisting', '/etc/passwd', '/etc'):
                ...
                self.assertEqual(index.repo.odb.stream(entries[0].binsha).read().decode('utf-8'), target)
    ...
    @skipIf(HIDE_WINDOWS_KNOWN_ERRORS and not PY3, r"""
        ...
        return path.replace('\\', '/')
        UnicodeDecodeError: 'utf-8' codec can't decode byte 0xc3 in position 0: ordinal not in range(128)""")
    @with_rw_directory
